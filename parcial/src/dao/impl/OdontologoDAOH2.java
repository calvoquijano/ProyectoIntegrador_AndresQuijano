...
public class OdontologoDAOH2 implements IDao<Odontologo> {
    ...
    private static final String SQL_UPDATE="UPDATE ODONTOLOGOS SET MATRICULA=?," +
            "NOMBRE=?, APELLIDO=? WHERE ID=?";
    private static final String SQL_DELETE="DELETE FROM ODONTOLOGOS WHERE ID=?";
    private static final Logger LOGGER=Logger.getLogger(OdontologoDAOH2.class);
    @Override
    public Odontologo guardar(Odontologo odontologo) {
        LOGGER.info("Iniciando el proceso de guardado del odontologo con nombre" +
                " igual= "+odontologo.getNombre());
    ...
    public Odontologo buscar(Integer id) {
        LOGGER.info("Iniciando el proceso de busqueda del odontologo con id" +
                " igual= "+id);
        Connection connection=null;
    ...
    public void actualizar(Odontologo odontologo) {
        LOGGER.info("Iniciando el proceso de actualización del odontologo con nombre" +
                " igual= "+odontologo.getNombre());
        Connection connection=null;
        try{
            connection=BD.getConnection();
    ...
    public void eliminar(Integer id) {
        LOGGER.warn("Iniciando el proceso de eliminación del odontologo con id" +
                " igual= "+id);
        Connection connection=null;
        try{
            connection=BD.getConnection();
